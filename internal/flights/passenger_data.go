...
func (s *Service) ProcessPassengerData(ctx context.Context, req PassengerDataRequest) (*PassengerDataResponse, error) {
	...
	session, err := s.sessionStore.Get(ctx, req.SearchID)
	if err != nil {
		return nil, fmt.Errorf("%w: search session expired or not found for ID: %s", ErrNotFound, req.SearchID)
	}
	expectedPassengers := session.Params.AdultCount + session.Params.ChildCount + session.Params.InfantCount
	...
	if _, ok := findOffer(session.TFOffers, req.OfferID); !ok {
	...
	if err := validatePassengerData(req, session); err != nil {
		return nil, err
	}

	profile, err := s.buildProcessTermsProfile(req, session)
	if err != nil {
		return nil, err
	}
...
func (s *Service) buildProcessTermsProfile(req PassengerDataRequest, session *FlightSearchSession) (travelfusion.BookingProfile, error) {
	...
	if req.OfferID == session.SelectedOfferID {
		if err := applyProcessTermsSnapshots(&profile, req, session.TFRequiredParameters); err != nil {
...
func applyProcessTermsSnapshots(profile *travelfusion.BookingProfile, req PassengerDataRequest, snapshots []TFRequiredParameterSnapshot) error {
...
func applyListBackedCSP(profile *travelfusion.BookingProfile, req PassengerDataRequest, snapshot TFRequiredParameterSnapshot) error {
...
func applyNameCSP(profile *travelfusion.BookingProfile, req PassengerDataRequest, snapshot TFRequiredParameterSnapshot) error {
	wireName, ok := knownSupplierParameterTFName(snapshot.Parameter)
	if !ok {
		return nil
	}
...
func buildNameCSPValue(parameter string, passenger Passenger) string {
...
func validatePassengerData(req PassengerDataRequest, session *FlightSearchSession) error {
	...
	if req.OfferID == session.SelectedOfferID {
		return validateRequiredDocumentCSPs(req, session.TFRequiredParameters)
	}
	return nil
}

func validateRequiredDocumentCSPs(req PassengerDataRequest, snapshots []TFRequiredParameterSnapshot) error {
	for _, snapshot := range snapshots {
		if isOptional(snapshot) {
			continue
		}
...
func validateRequiredSupplierParam(req PassengerDataRequest, snapshot TFRequiredParameterSnapshot, validate func(string) error) error {
	if boolValue(snapshot.PerPassenger) {
		for i := range req.Passengers {
			value, ok := firstPassengerSupplierParameter(req, i, snapshot.Parameter)
			if !ok {
...
func passengerName(passenger Passenger) travelfusion.Name {
...
func processTermsPhone(phone Phone) travelfusion.Phone {
...
func bookingMobilePhoneSupplierParameter(phone Phone) string {
...
func firstSupplierParameter(params []SupplierParameter, paramName string) (string, bool) {
...
func mapPassengerDataResponse(src *travelfusion.ProcessTermsResult) *PassengerDataResponse {
	if src == nil {
		return &PassengerDataResponse{SupplierResponses: []ProcessTermsSupplierResponse{}}
	}
	responses := make([]ProcessTermsSupplierResponse, len(src.SupplierResponses))
	for i := range src.SupplierResponses {
		responses[i] = ProcessTermsSupplierResponse{
