...
func (s *Service) cacheSeatMap(ctx context.Context, session *FlightSearchSession, offerID string, offer Offer, required []TFRequiredParameterSnapshot) {
	if session.TFSeatMapByOfferID == nil {
		session.TFSeatMapByOfferID = map[string][]SegmentSeatMap{}
	}

	rawSeatOptions, ok := extractSeatOptions(required)
	if !ok || !hasSeatOptions(rawSeatOptions) {
		delete(session.TFSeatMapByOfferID, offerID)
		return
	}

	seatMap := parseSeatOptions(rawSeatOptions, collectOfferSegments(offer))
	s.normalizeSeatPrices(ctx, seatMap)
	session.TFSeatMapByOfferID[offerID] = seatMap
}

func extractSeatOptions(required []TFRequiredParameterSnapshot) (string, bool) {
...
func collectOfferSegments(offer Offer) []Segment {
	segments := make([]Segment, 0, len(offer.OutboundFlight.Segments))
...
func parseSeatOptions(rawSeatOptions string, segments []Segment) []SegmentSeatMap {
	...
	result := make([]SegmentSeatMap, 0, len(segments))
	for i, segment := range segments {
		seats := []SeatDetail{}
		if i < len(blocks) && strings.TrimSpace(blocks[i]) != "" {
			seats = parseSeats(blocks[i])
		}
		result = append(result, SegmentSeatMap{
...
func parseSeats(block string) []SeatDetail {
	matches := seatEntryPattern.FindAllStringSubmatch(block, -1)
	seats := make([]SeatDetail, 0, len(matches))
...
func (s *Service) normalizeSeatPrices(ctx context.Context, seatMaps []SegmentSeatMap) {
